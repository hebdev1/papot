...
function InfoBox({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-[#F5E9D8] border border-[#e2d5c3] rounded-xl px-4 py-3 flex gap-3 items-start">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full mb-8">
        {PARTNER_TYPES.map(pt => (
          <div key={pt.id} className="bg-white border-2 border-[#e2d5c3] hover:border-[#6ad7fb] rounded-2xl p-6 text-left transition-all duration-200 hover:shadow-lg hover:shadow-[rgba(0,32,137,0.08)] group">
            <div className="w-14 h-14 rounded-2xl bg-[#F5E9D8] flex items-center justify-center text-3xl mb-4">{pt.emoji}</div>
        ...
        {PARTNER_TYPES.map(pt => {
          ...
          return (
            <button key={pt.id} onClick={() => onSelect(pt.id)}
              ...
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl shrink-0 ${isSelected ? "bg-[#e76f2e]/10" : "bg-[#F5E9D8]"}`}>{pt.emoji}</div>
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center mb-8">
        <div className="w-24 h-24 rounded-2xl bg-[#F5E9D8] border-2 border-dashed border-[#c8b9a5] flex flex-col items-center justify-center cursor-pointer hover:border-[#6ad7fb] transition-colors">
  ...
  if (partnerType === "hotel" || partnerType === "guesthouse") return (
    <div>
      ...
      {rooms.length === 0 && !addingRoom && (
        <div className="bg-[#F5E9D8] border-2 border-dashed border-[#c8b9a5] rounded-2xl p-12 text-center mb-6">
      ...
      {rooms.map(r => (
        <div key={r.id} className="bg-white border border-[#e2d5c3] rounded-xl p-4 mb-3 flex items-center gap-4">
          <div className="w-14 h-14 rounded-xl bg-[#F5E9D8] flex items-center justify-center text-2xl shrink-0">🛏️</div>
          <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-1 shrink-0">
            <button className="p-2 rounded-lg hover:bg-[#F5E9D8] text-[#7a6355] transition-colors"><Ico.Edit /></button>
    ...
    <div>
      ...
      {vehicles.map(v => (
        <div key={v.id} className="bg-white border border-[#e2d5c3] rounded-xl p-4 mb-3 flex items-center gap-4">
          <div className="w-14 h-14 rounded-xl bg-[#F5E9D8] flex items-center justify-center text-2xl shrink-0">🚗</div>
          <div className="flex-1 min-w-0">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {["Salle principale", "Terrasse", "Jardin", "VIP"].map(zone => (
            <div key={zone} className="bg-[#F5E9D8] rounded-xl p-3 text-center border border-[#e2d5c3]">
...
function StepPhotos({ partnerType, photos, onPhotos }: { partnerType: PartnerType; photos: File[]; onPhotos: (f: File[]) => void }) {
  ...
  return (
    <div>
      ...
      <div className="bg-[#F5E9D8] border border-[#e2d5c3] rounded-xl p-4 mb-6 flex items-start gap-3">
...
function StepReview({ partnerType }: { partnerType: PartnerType }) {
  ...
  return (
    <div>
      ...
      <div className="mt-6 bg-[#F5E9D8] rounded-xl p-4 flex items-center justify-between gap-4">
        <div>
...
function StepSuccess({ onDashboard, onPreview, onAdd }: { onDashboard: () => void; onPreview: () => void; onAdd: () => void }) {
  return (
    <div className="flex flex-col items-center text-center max-w-lg mx-auto py-8">
      ...
      <div className="bg-[#F5E9D8] border border-[#e2d5c3] rounded-xl px-6 py-3 mb-8 flex items-center gap-3">
...
function WizardShell({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[#F5E9D8]">
      {/* Top bar */}
