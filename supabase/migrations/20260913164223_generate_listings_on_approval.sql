-- Same generator, with one fix: most Haitian addresses name the same place as
-- commune and as city, and "Pétion-Ville, Pétion-Ville" reads as a mistake.
...
-- Accepting a file now also builds the catalogue it described, in the same
-- transaction as the partner and its owner. A partner that signs in for the
-- first time finds their rooms or their fleet already there, as drafts.
create or replace function public.admin_decide_application(p_id uuid, p_decision text, p_note text default null::text)
returns uuid
language plpgsql
security definer
set search_path to 'public', 'pg_temp'
...
begin
  ...
  if p_decision = 'accept' then
    ...
    -- The rooms, the fleet, the opening hours: everything the file already
    -- described becomes draft listings the partner can publish.
    v_listings := build_partner_listings(v_partner);
