drop function if exists public.submit_partner_application(jsonb);

/**
 * Strict submission: every value is typed, validated and written to a real
 * column. Credentials and payment instruments are never accepted at all —
 * the payload has no place to put them. Children go into their own tables,
 * so rooms/vehicles/hours/amenities carry their own constraints.
 * Any violation aborts the whole submission.
 */
...
begin
  ...
  insert into public.partner_applications (
    ...
    -- only the last four digits ever reach storage
  ...
  ---- amenities (catalog-checked by trigger) -------------------------------
  ...
  -- A lodging application without a single room is not a usable listing.
  if v_type in ('hotel', 'guesthouse') then
    ...
    if v_count = 0 then
      raise exception 'Ajoutez au moins une chambre avant de soumettre.';
    end if;
  end if;
  if v_type = 'car' then
    select count(*) into v_count from public.partner_application_vehicles where application_id = v_id;
    if v_count = 0 then
      raise exception 'Ajoutez au moins un véhicule avant de soumettre.';
    end if;
  end if;
